...
(function(){
    var m = Math, mround = function(r){
        return r >> 0;
    }, vendor = (/webkit/i).test(navigator.appVersion) ? 'webkit' : (/firefox/i).test(navigator.userAgent) ? 'Moz' : 'opera' in window ? 'O' : '',    // Browser capabilities
    ...
    RESIZE_EV = 'onorientationchange' in window ? 'orientationchange' : 'resize', START_EV = hasTouch ? 'touchstart' : 'mousedown', MOVE_EV = hasTouch ? 'touchmove' : 'mousemove', END_EV = hasTouch ? 'touchend' : 'mouseup', CANCEL_EV = hasTouch ? 'touchcancel' : 'mouseup', WHEEL_EV = vendor == 'Moz' ? 'DOMMouseScroll' : 'mousewheel',    // Helpers
    trnOpen = 'translate' + (has3d ? '3d(' : '('), trnClose = has3d ? ',0)' : ')',    // Constructor
    iScroll = function(el, options){
        ...
        that.options = {
            ...
            lockDirection: true,
            useTransform: true,
            useTransition: false,//建议不要使用useTransition
            topOffset: 0,
            ...
            // Events
            onRefresh: null,
            onBeforeScrollStart: function(e){	//放在_start函数中优先执行。
                e.preventDefault();
            },
            onScrollStart: null,//点击触发。只点击不会触发onScrollEnd //放在_start函数中最后执行。
            onBeforeScrollMove: null,//滚动前触发
            onScrollMove: null,//鼠标滚动触发。不论是否产生滚动效果，只要鼠标有滚动滑动的操作都会触发
            onBeforeScrollEnd: null,
            onScrollEnd: null,//滚动结束后触发
            onTouchEnd: null,//mouseup touchend触发
            onDestroy: null,
            onZoomStart: null,
            onZoom: null,
            onZoomEnd: null
    ...
    iScroll.prototype = {
        ...
        scale: 1,
        currPageX: 0,
        currPageY: 0,//当前页索引。对应li位置须+1
        pagesX: [],// 保存X轴页面
        pagesY: [],//Y轴页面
        aniTime: null,
        wheelZoomCount: 0,
        myFlag:false,//david的。用来判断滚动条是否滚动
        handleEvent: function(e){
            var that = this;
            switch (e.type) {
                case START_EV:	//touchestart mousedown事件
                    if (!hasTouch && e.button !== 0) 
                        return;
                    that._start(e);
                    break;
                case MOVE_EV://touchmove mousemove事件
                    that._move(e);
                    break;
                case END_EV://touchend mouseup事件
                case CANCEL_EV://touchcancel事件
                    that._end(e);
                    break;
                case RESIZE_EV://orientationchange 事件
                    that._resize();
                    break;
                case WHEEL_EV:
                    that._wheel(e);
        ...
        //touchstart和mousedown的处理函数
        _start: function(e){
            var that = this, point = hasTouch ? e.touches[0] : e, matrix, x, y, c1, c2;
            
            if (!that.enabled) 
                return;
            //david 如果动画未结束，则不接收其它事件处理
			//这里所有的滚动条都是enable的状态，导致滚动条同时滚动
//			console.log(that.wrapper.id + " enabled"+that.enabled);
//			console.log(that.wrapper.id + "start"+that.moved + that.animating + "," + that.absDistX + "X:"+ that.absDistY);	
			if(that.moved)
				return;
        ...
        _move: function(e){
            ...
            // Lock direction 重写该方法。比较XY轴滑动的距离，控制单个方向滑动
            if (that.options.lockDirection) {
                ...
                if (that.absDistX != 0 && that.absDistY != 0) {
                    if (that.absDistX > that.absDistY + 1) {//X轴滑动
                        newY = that.y;
                        deltaY = 0;
                    }
                    else if(that.absDistY > that.absDistX + 1){//Y轴滑动
                        newX = that.x;
                        deltaX = 0;
                    }
                }
        ...
        _end: function(e){
            ...
            if (!that.moved) {			
                if (hasTouch) {
                    if (that.doubleTapTimer && that.options.zoom) {//允许缩放
                        // Double tapped
        ...
        _startAni: function(){
            ...
            if (that.animating || that.moved) //david
                return;
            ...
            if (step.x == startX && step.y == startY) 
                step.time = 0;
            that.animating = true;			
            //that.moved = true;//david注释掉。因为在_start函数中判断moved为true就return。若这里为true，则会造成滚动条滚不动。这里注释掉，则滚动过程中点击，会停住滚动
            if (that.options.useTransition) {
                that._transitionTime(step.time);
                that._pos(step.x, step.y);
                that.animating = false;
				that.moved = false;//david添加
                if (step.time) 
...
//					//david
        ...
        refresh: function(){
			if(RESIZE_EV == "orientationchange"){
				if(window.orientation != 0 && window.orientation != 180) //横屏 david
        		return;
			}
            ...
            that.wrapperW = that.wrapper.clientWidth || 1;//wrapper标签的宽度
            that.wrapperH = that.wrapper.clientHeight || 1;
            
            that.minScrollY = -that.options.topOffset || 0;
            that.scrollerW = mround(that.scroller.offsetWidth * that.scale);
            that.scrollerH = mround((that.scroller.offsetHeight + that.minScrollY) * that.scale);
            that.maxScrollX = that.wrapperW - that.scrollerW;//滚动条的最大X轴位置。
            ...
            // Prepare snap	传入的是"li"等元素标签或className
            if (typeof that.options.snap == 'string') {
                that.pagesX = [];
                that.pagesY = [];
                els = that.scroller.querySelectorAll(that.options.snap);//查找所有标签或class为snap的元素
                ...
                if (that.options.snap) {	//snap:true。
                    that.pagesX = [];
					//按照wrapperW可以放下多少个scroll计算pagesX的个数。若wrapperW大于that.maxScrollX的绝对值，则只会生成一个长度，这样就有错误了。
                    while (pos >= that.maxScrollX) {
                        that.pagesX[page] = pos;
                        pos = pos - that.wrapperW;
                        page++;
                    }
                    if (that.maxScrollX % that.wrapperW) {//wrapperW是wrapper的宽度。
        ...
        scrollToPage: function(pageX, pageY, time){
            ...
            if (that.options.snap) {
                ...
                x = that.pagesX[pageX];//必须滚动scroll后。才会发生改变。否则x都是一个值。scrollToPage之前要调用refresh
                y = that.pagesY[pageY];
            }
            else {
            ...
            that.scrollTo(x, y, time || 400);//默认400毫秒的移动时间
        },
